...
impl<'i> Lexer<'i> {
   ...
   fn read_number(&mut self) -> Result<TokenKind, Error> {
      ...
      let start = self.position;
      if self.get() == b'0' && !matches!(self.get_ahead(1), b'0'..=b'9') {
         self.advance(); // skip 0
         match self.get() {
  ...
  level
    shenanigans
      ...
      let expected_kinds = [
         ...
         Integer(842..843), UpToInclusive, Integer(846..847),
         LeftParen, RightParen, LeftBracket, RightBracket, LeftBrace, RightBrace,
         Comma, Colon, Colons, Semicolon,
         Eof
      ];
      ...
      loop {
         ...
         if DEBUG {
            eprint!("{:#} |{}\t{:?}", token.span, token.indent_level, token.kind);
            if let Some(s) = token.kind.get_source(source) {
               eprint!("\t{:?}", s);
            } else if let TokenKind::Character(ch) = token.kind {
               eprint!("\t{:?}", char::from_u32(ch));
            } else if let Some(s) = token.kind.get_string(&lexer) {
               eprint!("\t{:?}", s);
            }
            eprintln!();
         }
         println!("{:?},  // {}", token.kind, token_index);
         assert_eq!(token.kind, expected_kinds[token_index]);
